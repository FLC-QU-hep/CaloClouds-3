...
class TestPointCloudDataset(AbsDatasetTest):
    ...
    def test_init(self):
        # For data0
        ds0 = self.ds_class(
            self.paths["data0"], bs=10, max_ds_seq_len=5, quantized_pos=False
        )
        assert len(ds0.open_files) == 1
        npt.assert_array_equal(ds0.index_list, [(5, 0, 0)])
        ...
        # For data1
        ds1 = self.ds_class(
            self.paths["data1"], bs=10, max_ds_seq_len=5, quantized_pos=False
        )
        ...
        npt.assert_array_equal(
            ds01.index_list, [(1, 1, 0), (1, 1, 2), (2, 1, 1), (5, 0, 0)]
        )
    ...
    def test_getitem(self):
        # For data0
        ds0 = self.ds_class(
            self.paths["data0"], bs=10, max_ds_seq_len=5, quantized_pos=True
        )
        batch = ds0[0]
        ...
        # For data1
        ds1 = self.ds_class(
            self.paths["data1"], bs=10, max_ds_seq_len=5, quantized_pos=True
        )
        batch = ds1[0]
...
class TestPointCloudDatasetRoll(AbsDatasetTest):
    ds_class = dataset.PointCloudDataset
    ds_class.metadata.layer_bottom_pos_hdf5 = np.linspace(0, 10, 30)
    ds_class.metadata.cell_thickness_hdf5 = 1 / 3
    rolled_axis = True
