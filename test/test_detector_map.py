...
def test_create_map():
    ...
    layers, offset = detector_map.create_map(X, Y, Z, layer_bottom_pos, half_cell_size, cell_thickness)
    npt.assert_allclose(offset, 1.)
...
def test_normalise_map():
    ...
    MAP = [{'xedges': x_edges_before, 'zedges': z_edges_before, 'grid': np.ones((4, 4))}]
