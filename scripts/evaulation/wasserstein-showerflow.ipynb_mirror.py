...
existing_list = []
for weight_decay in [0.0, 0.0001, 0.1]:
...
if os.path.exists(save_name):
    ...
    redo = True
    distances_1d = [
        np.zeros(n_cogs + n_pnts_layers + n_es_layers) for _ in existing_models["paths"]
    ]
...
if n_es_layers and redo:
    for ei, e in enumerate(
        range(n_cogs + n_pnts_layers, n_cogs + n_pnts_layers + n_es_layers)
    ):
        truth = g4_test[:].T[e]
        for i in range(n_models):
...
save_name = os.path.join(
    showerflow_utils.get_showerflow_dir(config), "sliced_wasserstein.npz"
)
...
for i, w in enumerate(working):
    ...
    n_blocks = existing_models["num_blocks"][w]
    plt.plot(
        [loss, loss], [distance - distance_err, distance + distance_err], lw=3, c=c
    )
...
for i, w in enumerate(working):
    ...
    n_blocks = existing_models["num_blocks"][w]
    plt.plot([auc, auc], [distance - distance_err, distance + distance_err], lw=3, c=c)
...
filter_xs = [
    existing_models["names"][w].split("_wo")[0] != "original_nb10_fnorms_dhist"
    for w in working
]
...
thing = plt_subax(
    axarr[1], params, aaucs, n_blocks, fnorms, "Number of parameters", "AUC"
)
...
thing = plt_subax(
    axarr, params, sliced, aaucs, fnorms, "Number of parameters", "Sliced Wasserstein"
)
...
thing = plt_subax(
    axarr[1], weight_decays, aaucs, n_blocks, fnorms, "Weight decay", "AUC"
)
...
weight_decays = np.array(existing_models["weight_decay"])[working]
thing = plt_subax(
    axarr, weight_decays, sliced, aaucs, fnorms, "Weight decay", "Sliced Wasserstein"
)
...
def get_loaded_flow(i):
    ...
    model, flow_dist, transforms = constructor(
        config_here.shower_flow_num_blocks, num_inputs, num_cond, "cpu"
    )
