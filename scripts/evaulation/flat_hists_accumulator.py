...
#    "../point-cloud-diffusion-logs/wish/dataset_accumulators/10-90GeV_x36_grid_regular_524k_float32/from_10.h5")
#
acc = StatsAccumulator.load(
    "../point-cloud-diffusion-logs/wish/dataset_accumulators/wish_v4.h5"
)
...
layer_thickness = np.mean(acc.layer_bottom[1:] - acc.layer_bottom[:-1])
zs = acc.layer_bottom + layer_thickness * 0.5
...
heatmap_dir = "../point-cloud-diffusion-images/stats_heatmap_wishv4"
print("Doing 2D")
for name in hists_2D:
    ...
    heatmap(
        arr, xs, zs, "Incident Energy", "Y (layers)", expand_name(name), cmap="viridis"
    )
    plt.tight_layout()
    save_ensure_dir_exists(f"{heatmap_dir}/{name}.")
...
xs = (
    acc.Xmin - acc.lateral_bin_size * 0.5,
    acc.Xmax + acc.lateral_bin_size * 0.5,
)
ys = (
    acc.Ymin - acc.lateral_bin_size * 0.5,
    acc.Ymax + acc.lateral_bin_size * 0.5,
)
    ...
    for incident_bin in range(first_bin, last_bin):
        ...
        for layer in range(acc.n_layers):
            ...
            save_ensure_dir_exists(
                f"{heatmap_dir}/{name}_layers/global/"
                f"E{incident_energy_range[0]:02}_layer{layer:02}_global."
            )
            ...
            plt.tight_layout()
            save_ensure_dir_exists(
                f"{heatmap_dir}/{name}_layers/perInci/"
                f"E{incident_energy_range[0]:02}_layer{layer:02}_perInci."
            )
            ...
            plt.tight_layout()
            save_ensure_dir_exists(
                f"{heatmap_dir}/{name}_layers/local/"
                f"E{incident_energy_range[0]:02}_layer{layer:02}_local."
            )
