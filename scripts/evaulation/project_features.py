...
for j in range(len(min_energy_list)):
    ...
    if full_spectrum:
        ...
        path = "/beegfs/desy/user/akorol/data/calo-clouds/hdf5/all_steps/validation/photon-showers_{}GeV_A90_Zpos4.slcio.hdf5".format(
            min_energy
        )
    ...
    i = 0
    f = out_path + "{}-{}GeV_{}_{}_seed{}.npz".format(
        str(min_energy),
        str(max_energy),
        str(n_events),
        caloclouds_list[i],
    ...
    MAP, _ = create_map()
    events, cloud = get_projections(
        real_showers[:n], MAP, max_num_hits=6000, return_cell_point_cloud=True
    )
    events_fake, cloud_fake = get_projections(
        fake_showers[:n], MAP, max_num_hits=6000, return_cell_point_cloud=True
    )
    events_fake_2, cloud_fake_2 = get_projections(
        fake_showers_2[:n], MAP, max_num_hits=6000, return_cell_point_cloud=True
    )
    events_fake_3, cloud_fake_3 = get_projections(
        fake_showers_3[:n], MAP, max_num_hits=6000, return_cell_point_cloud=True
    )
    ...
    real_list, fakes_list = plotting.get_observables_for_plotting(
    ...
    with open(
        pickle_path + "dict_{}-{}GeV.pickle".format(str(min_energy), str(max_energy)),
        "wb",
    ) as f:
        pickle.dump(dict, f)
