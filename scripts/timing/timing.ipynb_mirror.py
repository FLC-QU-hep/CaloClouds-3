...
cc2_ddsim_fit = np.polyval(np.polyfit(cc2_energies, cc2_per_event_no_setup, 4, w=1/cc2_err), xs)
cc3_ddsim_fit = np.polyval(np.polyfit(cc3_energies, cc3_per_event_no_setup, 4, w=1/cc3_err), xs)
g4_ddsim_fit = np.polyval(np.polyfit(g4_energies, g4_per_event_no_setup, 4, w=1/g4_err), xs)

for ax in [ax1]: 
    ax.errorbar(g4_energies, g4_per_event_no_setup, g4_err, marker='o', linestyle='none', ecolor='k', c='k', alpha=0.3)

    ax.plot(xs, g4_ddsim_fit, c='k', label="fit to G4")
    
    
    ax.errorbar(cc2_energies, cc2_per_event_no_setup, cc2_err, marker='o', linestyle='none', ecolor='tab:blue', c='tab:blue', alpha=0.3)
    ax.plot(xs, cc2_ddsim_fit, c='tab:blue', label="fit to CC2")
    ax.errorbar(cc3_energies, cc3_per_event_no_setup, cc3_err, marker='o', linestyle='none', ecolor='tab:orange', c='tab:orange', alpha=0.3)
...
ax4.plot(xs, g4_ddsim_fit/cc3_ddsim_fit, c='tab:orange', label=f"mean(time(G4)/time(CC2) = {np.mean(g4_ddsim_fit/cc3_ddsim_fit):.4}")
ax4.plot(xs, cc2_ddsim_fit/cc3_ddsim_fit, c='tab:green', label=f"mean(time(CC2)/time(CC3) = {np.mean(cc2_ddsim_fit/cc3_ddsim_fit):.4}")
ax4.set_ylabel("Times faster")
ax4.legend()
fig.suptitle("All models run in ddsim using a single CPU thread")
plt.tight_layout()
