...
from pointcloud.evaluation.calculate_scale_factors import (
    construct_dsf,
    get_path,
    DetectorScaleFactors,
)
...
if os.path.exists(save_name) and False:
    ...
    dsf1 = construct_dsf(
        ...
        models[model_name][0],
        models[model_name][1],
        model_gun_pos=model_gun,
        g4_gun_pos=g4_gun,
        max_g4_events=n_events,
    )
    dsf1.save(save_name)
...
if os.path.exists(save_name_2) and False:
    ...
    dsf_2 = construct_dsf(
        config,
        model_name_2,
        models[model_name_2][2],
        models[model_name_2][0],
        models[model_name_2][1],
        model_gun_pos=model_gun,
        g4_gun_pos=g4_gun,
        max_g4_events=n_events,
    )
...
def plot_sample(
    dsf, model_name, n_samples, sample_n_coeff, sample_e_coeff=None, fake_n_coeff=None
):
    mean_sq_n, mean_sq_e = dsf.divergance(
        n_samples, sample_n_coeff, sample_e_coeff, fake_n_coeff
    )
...
sample_n_coeff[-2] = 1.0
sample_e_coeff = np.array([1.0, 0.0])
sample_e_coeff_2 = np.array([1.0, 0.0])
...
sample_n_coeff = np.zeros(4)
sample_n_coeff[-2] = 1.0
sample_e_coeff = np.array([5.486e-01, 0.0])
sample_e_coeff_2 = np.array([9.768e-01, 0.0])
plot_sample(dsf1, model_name, n_samples, sample_n_coeff, sample_e_coeff)
plot_sample(dsf_2, model_name_2, n_samples, sample_n_coeff, sample_e_coeff_2)
...
for dsf in [dsf1, dsf_2]:
    ...
    fig.suptitle(
        f"energy={dsf.retain_dict['cond_energy'][i][0]}, points={dsf.retain_dict['cond_points'][i][0]}, p_norm_local={dsf.retain_dict['cond_p_norm_local'][i][:]}"
    )
    vmin, vmax = 0.0001, 1
    vmin2, vmax2 = 0.1, 100
    ...
    axarr[0].scatter(
        g4_evt[:, x], g4_evt[:, y], c=g4_evt[:, c], norm=LogNorm(vmin=vmin2, vmax=vmax2)
    )
    axarr[1].matshow(
        dsf.retain_dict["g4_projected_postMip"][i][l],
        norm=LogNorm(vmin=vmin, vmax=vmax),
    )
    axarr[2].scatter(
        md_evt[:, x], md_evt[:, y], c=md_evt[:, c], norm=LogNorm(vmin=vmin2, vmax=vmax2)
    )
    axarr[3].matshow(
        dsf.retain_dict["model_projected_postMip"][i][l],
        norm=LogNorm(vmin=vmin, vmax=vmax),
    )
labels = ["g4", "CC2", "CC3"]
before = [dsf1.retain_dict["g4_events"]] + [
    dsf.retain_dict["model_events"] for dsf in [dsf1, dsf_2]
]
after = [dsf1.retain_dict["g4_projected"]] + [
    dsf.retain_dict["model_projected"] for dsf in [dsf1, dsf_2]
]
before_cogs = []
after_cogs = []
...
for i in range(3):
    b = np.array(before[i])
    m = (b[:, :, 3] > 0)[:, :, None]
    cogs = np.sum((m * b[:, :, :3] * b[:, :, [3]]), axis=1) / np.sum(
        m * b[:, :, [3]], axis=1
    )
...
if hasattr(dsf1, "final_e_coeff") and not redo:
    ...
    def to_minimise_1b(coeff_e):
        ...
        mean_sq_n, mean_sq_e = dsf1.divergance(
            n_samples, real_coeff, sample_e_coeff, fake_coeff
        )
        return mean_sq_e

    res_e_coeff_1 = minimize(
        to_minimise_1b,
        x0=1.0,
        method="Powell",
        bounds=[
        ...
        options={"disp": True, "maxiter": 1000, "xtol": 1e-6, "ftol": 1e-4},
    )
    print(res_e_coeff_1)
...
if hasattr(dsf_2, "final_n_coeff") and not redo:
    ...
    res_n_coeff_2 = minimize(
        ...
        options={"disp": True, "maxiter": 1000, "xtol": 1e-6, "ftol": 1.0},
    )
    print(res_n_coeff_2)
