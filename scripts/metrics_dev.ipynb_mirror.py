...
MAP = create_map(config=cfg)
real_events = get_projections(real_showers[:], MAP, config=cfg)
len(real_events)
len(real_events[0])
importlib.reload(plotting)
...
st = time.time()
dict = plotting.get_features(plt_config, MAP, metadata.half_cell_size_global, real_events[:], metadata.global_shower_axis_char)
print(time.time()-st)
print(dict.keys())
