...
from pointcloud.utils.stats_accumulator import AlignedStatsAccumulator, save_location, HighLevelStats
...
ye_sample = np.zeros((check_events, configs.max_points*10, 2))
...
for start_idx in range(0, check_events, batch_size):
    ...
    ye_sample[start_idx:end_idx, :n_pts, 0][mask] = events[..., 1][mask]

    ye_sample[start_idx:end_idx, :n_pts, 1][mask] = e[mask]

ye_sample[..., 0] -= shifts[:, 1, None]
...
y_values = ye_sample[..., 0][ye_sample[..., 1]>0]
...
for row, (lower, upper) in enumerate(zip(shift_bins[:-1], shift_bins[1:])):
    ...
    subsample = ye_sample[shift_mask]
...
summed_xz_hits = np.sum(acc.counts_hist, axis=(0, 1))

summed_x_hits = np.sum(acc.counts_hist, axis=(0, 1, 3))

summed_z_hits = np.sum(acc.counts_hist, axis=(0, 1, 2))

x_bin_centers, _, z_bin_centers = acc._get_bin_centers()

bin_area = acc.lateral_bin_size**2

x_mean = np.sum(x_bin_centers*summed_x_hits)/np.sum(summed_x_hits)

z_mean = np.sum(z_bin_centers*summed_z_hits)/np.sum(summed_z_hits)

print(f"x mean = {x_mean}, z mean = {z_mean}")

z_mean = 0.05

shifted_z_bin_centers = z_bin_centers - z_mean
bin_radii = np.sqrt(x_bin_centers[:, None]**2 + shifted_z_bin_centers**2)
