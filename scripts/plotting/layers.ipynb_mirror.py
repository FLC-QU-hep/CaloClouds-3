...
ys = events[:, :, 1][mask].flatten()

floors, ceilings = floors_ceilings(metadata.layer_bottom_pos_raw, metadata.cell_thickness_raw, .5)



fig, ax = plt.subplots(figsize=(20, 3))

weights, bins, patches = ax.hist(ys, bins=300, color='k')
...
ax.vlines(metadata.layer_bottom_pos_raw, 0, mid, color='blue', alpha=0.5)
...
floors, ceilings = floors_ceilings(layer_bottom_pos, cell_thickness)
...
ys_normed = normed_events[:, :, 1][mask].flatten()



fig, ax = plt.subplots(figsize=(20, 3))

weights, bins, patches = ax.hist(ys_normed, bins=300, color='k')
