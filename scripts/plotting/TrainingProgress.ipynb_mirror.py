...
def get_pure_output_paths(history_path):
    output_dirs = [
        "/data/dust/user/dayhallh/CC2-out/",
    ...
    for output_dir in output_dirs:
        paths += [
            ...
            if prefix in f and f.endswith("npz")
        ]
...
def get_cog_values(prefix, paths, cog_idx=2, npz_format=True):
    iteration = np.fromiter(
        (int(f.split(prefix)[-1].split("_")[0]) for f in paths), dtype=int
    )
    ...
    **kwargs,
    ...
    for h, history_path in enumerate(history_paths):
        ...
        for i, ax in enumerate(ax_arr[:2].flatten()):
            ax.plot(
                epoch, history[i + 1], c=colours[h], alpha=0.6, label=history_labels[h]
            )
...
fig, ax_arr = plot_history(
    [history_path, history_path3, history_path4],
    "Comparison",
    history_labels=[title, title3, title4],
)
...
fig, ax_arr = plot_history(
    [history_path1, history_path2, history_path3], title, history_labels=labels
)
...
fig, ax_arr = plot_history(
    [history_path1], title, ax0_ylim=(-200, 50), ax3_ylim=(0, 7_000)
)
history_path = "/data/dust/user/dayhallh/point-cloud-diffusion-data/showerFlow/highGran_g40_p22_th90_ph90_en10-100/ShowerFlow_original_nb10_inputs36893488147419103231_dhist_try8_history.npy"
title = "Original, high granularity"
...
fig, ax_arr = plot_history(
    [history_path, history_path3], "Comparison", history_labels=[title, title3]
)
