...
class EPiClayer_cond2(nn.Module):
    def __init__(self, local_in_dim, hidden_dim, latent_dim, cond_feats=1, sum_scale=1e-3):
        super().__init__()
        self.act = F.leaky_relu
        self.cond_feats = cond_feats
        self.fc_global1 = weight_norm(nn.Linear(hidden_dim+hidden_dim+latent_dim+cond_feats, hidden_dim)) 
        self.fc_global2 = weight_norm(nn.Linear(hidden_dim, latent_dim)) 
        self.fc_local1 = weight_norm(nn.Linear(local_in_dim+latent_dim+cond_feats, hidden_dim))
        self.fc_local2 = weight_norm(nn.Linear(hidden_dim, hidden_dim))
...
class EPiC_encoder_cond(nn.Module):
    def __init__(self, zdim, input_dim=4, cond_features=1):
        ...
        self.fc_l1 = weight_norm(nn.Linear(self.feats+self.cond_feats, self.hidden_dim))
        self.fc_l2 = weight_norm(nn.Linear(self.hidden_dim, self.hidden_dim))

        self.fc_g1 = weight_norm(nn.Linear(self.hidden_dim+self.hidden_dim+self.cond_feats, self.hidden_dim))
        self.fc_g2 = weight_norm(nn.Linear(self.hidden_dim, self.latent))

        self.nn_list = nn.ModuleList()
        for _ in range(self.equiv_layers):
            self.nn_list.append(EPiClayer_cond2(self.hidden_dim, self.hidden_dim, self.latent, cond_feats=self.cond_feats, sum_scale=self.sum_scale))
        
        self.fc_g3 = weight_norm(nn.Linear(self.hidden_dim+self.hidden_dim+self.latent+self.cond_feats, self.hidden_dim))
        self.fc_g4 = weight_norm(nn.Linear(self.hidden_dim, self.hidden_dim))

        self.fc_g_out_mean = weight_norm(nn.Linear(self.hidden_dim, self.latent))
        self.fc_g_out_logvar = weight_norm(nn.Linear(self.hidden_dim, self.latent))
