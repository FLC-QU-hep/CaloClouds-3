...
def get_shower(
    model, num_points, energy, cond_N, bs=1, kdiffusion=False, config=Configs()
):
...
def gen_showers_batch_OLD(
    ...
    samples = (
        shower_flow.condition(cond_E / 100)
        .sample(
            torch.Size(
                [
                    num,
    ...
    energies = np.clip(
        (samples[:, 1] * 2.5 * 1000).reshape(num, 1), 0.04 * 1000, None
    ...
    for evt_id in tqdm(range(0, num, bs), disable=False):
        ...
        hits_per_layer_all = clusters_per_layer_gen[
            evt_id : evt_id + bs
        ]  # shape (bs, num_layers)
        e_per_layer_all = e_per_layer_gen[
            evt_id : evt_id + bs
        ]  # shape (bs, num_layers)
    ...
    fake_showers = np.moveaxis(
        fake_showers, -1, -2
    )  # (bs, num_points, 4) -> (bs, 4, num_points)
    ...
    # CoG calibration
    cog = get_cog(
        fake_showers[:, 0], fake_showers[:, 1], fake_showers[:, 2], fake_showers[:, 3]
    )
