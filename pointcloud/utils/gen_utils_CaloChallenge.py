...
def gen_showers_batch(model, shower_flow, e_min, e_max, config, num=2000, bs=32):
    ...
    for evt_id in tqdm(range(0, num, bs)):
        ...
        fs = get_shower(model, max_num_clusters, cond_E[evt_id : evt_id+bs], cond_N, bs=bs, config=config)
...
#     #reverse process to normalization
#     cog_x = samples[:, 2] * 25      # scaling numbers
#     cog_y = samples[:, 3] * 15 + 15
#     cog_z = samples[:, 4] * 20 + 40
#     #no masking for cog cause we sort the energy already
...
#     fake_showers[:, 2] -= (cog[2] - cog_z)[:,None] #for y we have n points for layer, we already did n_point_per layer. We dont noeed calibration
    
#     output['showers'] = fake_showers
    
#     return output

# # TODO: new version of gen_showers_batch from CaloClouds, to be adapted for CaloChallenge
# # def gen_showers_batch_NEW(
# #     model,
# #     shower_flow,
# #     e_min,
# #     e_max,
...
#         )
#         e_per_layer_all = (
#             e_per_layer_gen  # [evt_id : evt_id+bs] # shape (bs, num_layers)
#         )
#         max_num_clusters = hits_per_layer_all.sum(axis=1).max()
#         cond_N = (
...
#         fs[:, 0] = fs[:, 0] * (metadata.Xmin - metadata.Xmax) + metadata.Xmax
#         fs[:, 2] = fs[:, 2] * (metadata.Zmin - metadata.Zmax) + metadata.Zmax
