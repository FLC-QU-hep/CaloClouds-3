...
def percentile_edges_layer_occupancy(occ_layers):
    ...
    for i in range(len(occ_layers_list_sum)):
        ...
        if (
            running_fraction >= fraction_interval_min
        ):  # and (running_fraction <= fraction_interval_max):
...
def percentile_edges_radial_occupancy(occ_radial, min_radius=0, max_radius=300):
    ...
    for radius in np.arange(0, 300, 1e-3):
        ...
        running_fraction = running_sum / total
        if (running_fraction > fraction_interval_min) and (
            running_fraction <= fraction_interval_max
        ):
            fraction_interval_min += 0.1
            fraction_interval_max += 0.1
        ...
        for key, value in d.items():
            if key not in key_exceptions:
                if key in merged_dict:
                    # Assuming you want to concatenate lists or
                    # concatenate numpy arrays for duplicate keys.
                    if isinstance(merged_dict[key], list):
                        merged_dict[key].extend(value)
                    elif isinstance(merged_dict[key], np.ndarray):
                        if len(merged_dict[key].shape) == 1:
                            merged_dict[key] = np.concatenate(
                                (merged_dict[key], value), axis=0
                            )
                        elif len(merged_dict[key].shape) == 2:
                            merged_dict[key] = np.concatenate(
                                (merged_dict[key], value), axis=1
                            )
    ...
    for i in range(obs_real.shape[1]):
        ...
        for _ in range(iterations):
            wdist = wasserstein_distance(
                obs_real[j : j + batch_size, i], obs_model[j : j + batch_size, i]
            )
            wdists.append(wdist)
            j += batch_size
