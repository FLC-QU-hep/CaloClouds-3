...
class AllCond_epicVAE_nFlow_PointDiff(Module):
    def __init__(self, args):
        ...
        self.encoder = EPiC_encoder_cond(
            args.latent_dim, input_dim=args.features, cond_features=args.cond_features
        )
        self.flow = get_flow_model(args)
        if args.use_epic:
            self.diffusion = DiffusionPoint(
                net=PointwiseNet_epic_res_wn(
...
class PointwiseNet_epic_res_wn(Module):
    def __init__(self, point_dim, context_dim, residual):
        ...
        self.layers = ModuleList(
            [
                ConcatSquashEPiC_wn(
                    point_dim, 128, context_dim + 3 + context_dim + 3
                ),  # gated learned sum of point vector and context vector
