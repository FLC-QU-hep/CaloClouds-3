...
class EPiClayer_cond2(nn.Module):
    def __init__(
        self, local_in_dim, hidden_dim, latent_dim, cond_feats=1, sum_scale=1e-3
    ):
        super().__init__()
        self.act = F.leaky_relu
        self.cond_feats = cond_feats
        self.fc_global1 = weight_norm(
            nn.Linear(hidden_dim + hidden_dim + latent_dim + cond_feats, hidden_dim)
        )
        self.fc_global2 = weight_norm(nn.Linear(hidden_dim, latent_dim))
        self.fc_local1 = weight_norm(
            nn.Linear(local_in_dim + latent_dim + cond_feats, hidden_dim)
        )
        self.fc_local2 = weight_norm(nn.Linear(hidden_dim, hidden_dim))
        self.sum_scale = sum_scale

    def forward(
        self, x_global, x_local, cond_feats
    ):  # shapes: x_global[b,latent], x_local[b,n,latent_local]  cond_feats[B,C]
        ...
        x_pooledCATglobal = torch.cat(
            [x_pooled_mean, x_pooled_sum, x_global, cond_feats], 1
        )
        x_global1 = self.act(
        ...
        cond_feats2local = cond_feats.view(-1, 1, self.cond_feats).repeat(
            1, n_points, 1
        )  # B,N,C
...
class EPiC_encoder_cond(nn.Module):
    def __init__(self, zdim, input_dim=4, cond_features=1):
        ...
        self.fc_l1 = weight_norm(
            nn.Linear(self.feats + self.cond_feats, self.hidden_dim)
        )
        self.fc_l2 = weight_norm(nn.Linear(self.hidden_dim, self.hidden_dim))

        self.fc_g1 = weight_norm(
            nn.Linear(
                self.hidden_dim + self.hidden_dim + self.cond_feats, self.hidden_dim
            )
        )
        ...
        self.fc_g3 = weight_norm(
            nn.Linear(
                self.hidden_dim + self.hidden_dim + self.latent + self.cond_feats,
                self.hidden_dim,
            )
        )
