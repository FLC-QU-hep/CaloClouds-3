...
it may be rescaled/translated before it's saved to disc, and the coordinates it's saved to disc with are referred to as `raw`.

**Individual files;**

- `gun_xyz_pos`: `[   0.  1804.7  -50. ]` Position of the particle gun in physical coordinates.
- `gun_xz_pos_raw`: `[  0. -50.]` Position of the particle gun in the coordinates the data is saved in. This may be rescaled from physical units.
- `layer_bottom_pos`: `[1811.34020996 1814.46508789 1823.81005859 1826.93505859 1836.2800293 ... ]` y-coordinate of the bottom of each layer, physical coordinates.
- `layer_bottom_pos_raw`: `[1811.34020996 1814.46508789 1823.81005859 1826.93505859 1836.2800293 ... ]` Same as above, but saved coordinates.
...
- `half_cell_size`: `2.5441665649414062` Half the physical cell size (axis parallel to detector circumference)
- `half_cell_size_raw`: `2.5441665649414062` Same as previous, but in the units the data is saved in.
- `cell_thickness`: `0.5250244140625` The thickness of the cell in physical dimensions in the radial direction.
- `cell_thickness_raw`: `0.5250244140625` Same as previous, but in the units the data is saved
...
- `local_xyz_orientation`: `False` ??

Rescales, used when learning distributions like showerflow.

- `incident_rescale`: `100` 
- `vis_eng_rescale`: `2.5`
- `mean_cog`: `[ 3.80404145e-01  1.89524914e+03 -4.99723479e+01]`
- `std_cog`: `[ 0.51969929 12.76039063  0.48370289]`
- `n_pts_rescale`: `5000`
...
print(f"The half_cell_size_raw is {metadata.half_cell_size_raw} and " +
      f"the bottom y coordinates of the first 3 layers are {metadata.layer_bottom_pos_raw[:3]}")
```

Which prints something like;
> The half_cell_size_raw is 2.5441665649414062 and the bottom y coordinates of the first 3 layers are [1811.34020996 1814.46508789 1823.81005859]
...
```python
import numpy as np
np.save("pointcloud/metadata/gun_xyz_pos.npy", [0., 1804.7, -50.])  # saved as a seperate file
np.save("pointcloud/metadata/gun_xz_pos_raw.npy", [0., -1.])
box_edges = {"Ymin": 1811, "Xmin": -200, "Xmax": 200, "Zmin": -260, "Zmax": 140}
...
The corrisponding attributes just won't be created in the `Metadata` object, and
if they are needed, you will see an `AttributeError` when you run the code.
Should you get an `AttributeError`, you might need to find out what the corrisponding value actually is...
