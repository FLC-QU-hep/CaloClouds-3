...
Configs are stored in `configs/`, and the one in use is determined by what the soft link `configs.py` is pointing to.
...
The teacher model is trained using `main.py`, and the student model is trained with `python cd.py`.
